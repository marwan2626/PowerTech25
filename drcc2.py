...
def calculate_sensitivity_trafo(heatpumpForecast, opf_results, time_steps, transformer_capacity_mw):
    # Check if 'line_results' exists in opf_results
    ...
    w_t_prev = float(heatpumpForecast['stdP_NORM'].iloc[0])
    ...
    for t in range(1, len(time_steps)):
        ...
        sensitivity_trafo_value =  (100*abs(w_t-w_t_prev)) / (transformer_loading_t *transformer_capacity_mw)
        #print(f"time step:{t}, transformer loading:{transformer_loading_t} - previous transformer loading: {transformer_loading_prev}")
        sensitivity_trafo_results[time_steps[t]] = float(sensitivity_trafo_value)
        print(f"time step:{t}, w_t:{w_t} - w_t_prev: {w_t_prev} = sensitivity_trafo_value: {sensitivity_trafo_value}")
...
def calculate_omega_P_trafo(alpha, sensitivity_trafo, cov_matrix, Omega_P_trafo, time_steps, transformer_capacity_mw):
    print('Calculating omega P trafo')
    ...
    # Compute scaling factor
    scaling_factor = np.sqrt((1 - alpha) / (alpha)) # 95% confidence interval
    ...
    for t in time_steps[1:]:
        ...
        # Calculate the L2 norm of the scaled sensitivity vector
        Omega_P_trafo_new[t] = 2 * scaling_factor * np.linalg.norm(scaled_trafo_sensitivity)
...
def solve_opf(net, time_steps, const_load_heatpump, const_load_household, heatpump_scaling_factors_df, T_amb, Bbus, Omega_P, Omega_P_Trafo):
    ...
    for t in time_steps:
        ...
        transformer_loading_vars[t] = model.addVar(lb=0, ub=transformer_capacity_mw-Omega_P_Trafo[t], name=f'transformer_loading_{t}')
