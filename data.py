...
def sol_import(filename):
    rltDir = 'rlt/case%s_t%s_loadVar%s_pvVar%s_%s_cc%s_drcc%s_flx%s_%s_bat%s/'\
                %(pm.N_BUS,pm.T,pm.FLGVAR_LOAD,pm.FLGVAR_PV,pm.FCSTCASE[0],\
                  pm.FLGCC, pm.FLGDRCC,pm.FLGSHIFT,pm.UNBALANCE,pm.FLGBAT)
    #rltDir = 'rlt/case%s_t%s_loadVar%s_pvVar%s_%s_cc1_drcc1_flx%s_%s_bat%s/'\
                #%(pm.N_BUS,pm.T,pm.FLGVAR_LOAD,pm.FLGVAR_PV,pm.FCSTCASE[0],\
                  #pm.FLGSHIFT,pm.UNBALANCE,pm.FLGBAT)
...
class sets:
    ...
    ts = list(np.where(batData.values[:,7]>0,1,0)) # ts node
    ...
    idxHS = idx_list(phase_multiplication(ts), n*pm.N_PH) # set of bat Buses
...
class load:
    ...
    hPeak = loadData.values[:,6]/(pm.S_BASE*1e3)
    ...
    # active & reactive power HP demand [p.u]
    pHD = np.zeros((n*pm.N_PH,pm.T,loadCase))
    qHD = np.zeros((n*pm.N_PH,pm.T,loadCase))
    for c in range(loadCase):
        for i in range(n):
            for j in range(pm.N_PH):
                if pm.FLGLOAD == 1:
                    # active power demand
                    pHD[i*pm.N_PH+j,:,c] = pm.LOADCASE[c]*pm.LOADSHARE[j]*\
                                            hPeak[i]*pf[i]*\
                                            load_profile(profile[i])
                                        
                    # reactive power demand
                    qHD[i*pm.N_PH+j,:,c] = pm.LOADCASE[c]*pm.LOADSHARE[j]*\
                                            hPeak[i]*np.sin(np.arccos(pf[i]))*\
                                            load_profile(profile[i])
                else:
                    pHD[i*pm.N_PH+j,:,c] = pm.LOADCASE[c]*pm.LOADSHARE[j]*\
                                            hPeak[i]*pf[i]
...
class COP:
    # normalize load profiles & assign to node
    def COP_profile(i):
        profile = pd.read_csv('COP_curve.csv')
        
        # discretized load profile into T steps
        nMeasure = int(24/pm.TIMESTEP)
        profile_disc = np.array([np.mean(profile[j*int(pm.TIMESTEP*60):\
                                                      (j+1)*int(pm.TIMESTEP*60)])\
                                 for j in range(nMeasure)]) 
        ...
        # export
        profile_COP = profile_disc[t_start:t_end]
        
        return profile_COP
    
    COP_HP = np.zeros((n*pm.N_PH,pm.T,1))
    for i in range(n):
        COP_HP[i,:, 0] = COP_profile(i)[:,0] 
...
### TS ###
class ts: 
    icTS = pm.FLGBAT*batData.values[:,7]/pm.S_BASE # installed capacity [p.u.]
    etaTS = batData.values[:,2] # efficiency
    capMin = batData.values[:,3] # cap min 
    capMax = batData.values[:,4] # cap max
    capInit = batData.values[:,5] # initial cap
    e2p = batData.values[:,6] # energy-to-power ratio [MWh/MW]
    
    # bounds
    pFill_ub = pm.FLGBAT*(sets.ts*icTS/e2p) # battery charging
    pEmp_ub = pm.FLGBAT*(sets.ts*icTS*10/e2p) # battery discharging
    hTS_ub = icTS*capMax # soc max 
    hTS_lb = icTS*capMin # soc min
...
class hd:
    ...
    ### CHECK IF FORECAST FILE EXISTS ###
    fcstFile = 'src/fcst/forecastHD_v%s_%s_t%s.pkl'%(pm.V_FCST,pm.FCSTCASE[0],pm.T)
    if os.path.exists(fcstFile):
        ### READ FCST FILE ###
        file = open(fcstFile, 'rb') # open results file
        hdFcst = pkl.load(file) # create arry from file
        file.close() # close file
    else:
        ### RUN FORECAST ###
        print('Run forecasting script ...')
    ...
    dataFcst = hdFcst[3] # all forecast data
    nSamples = np.size(hdFcst[3],1) # number of samples
    ...
    # forecasted HD per phase
    pHD = np.zeros((n*pm.N_PH,pm.T,pvCase*loadCase))
    pHD = np.append(np.zeros((pm.N_PH,pm.T)),\
                            np.dot(icPhase.reshape(l*pm.N_PH,1),\
                                    hdFcst[0].reshape(1,pm.T)),axis=0)

    
    ### COVARIANCE MATRIX ###
    # covariance matrix for all timesteps
    cov = np.zeros((l*pm.N_PH,l*pm.N_PH,pm.T))
    for t in range(pm.T):        
        # full covariance matrix
        cov[:, :,t] = np.cov(np.dot(icPhase.reshape(l*pm.N_PH,1),\
                            (0.01*dataFcst[t,:]).reshape(1,nSamples)))

    # delete empty columns and rows
    rowDel = []
    for i in range(l*pm.N_PH):
        if np.all(cov[i,:,int(pm.T/1.2)] == 0):
            rowDel.append(i)
    covRed = np.delete(cov,rowDel,1)
    covRed = np.delete(covRed,rowDel,0)
